...
gmd(
  {
    pattern: "freezelastseen",
    aliases: ["lastseenfreeze", "hidelastseen"],
    react: "🧊",
    category: "owner",
    description: "Stop WhatsApp's last-seen timestamp from updating (on/off)",
  },
  async (from, Guru, conText) => {
    const { q, reply, react, isSuperUser } = conText;
    if (!isSuperUser) return reply("❌ Owner Only Command!");
    const state = (q || "").toLowerCase().trim();
    if (state !== "on" && state !== "off") {
      return reply("❌ Usage: .freezelastseen <on|off>");
    }
    const value = state === "on" ? "true" : "false";
    try {
      const current = await getSetting("FREEZE_LAST_SEEN");
      if ((current || "false") === value) {
        return reply(`⚠️ Freeze last-seen is already *${state.toUpperCase()}*.`);
      }
      await setSetting("FREEZE_LAST_SEEN", value);
      await react("✅");
      await reply(
        state === "on"
          ? "🧊 Last-seen frozen — the bot will stop broadcasting \"online\" presence, so your last-seen timestamp stays put instead of updating in real time."
          : "✅ Last-seen unfrozen — normal presence behavior resumed.",
      );
    } catch (error) {
      await reply(`❌ Error: ${error.message}`);
    }
