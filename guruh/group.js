...
gmd(
  ...
  async (from, Guru, conText) => {
    ...
    if (groupMetadata?.participants) {
      ...
      if (participant?.admin === "superadmin") isSuperAdminTarget = true;
      if (participant?.admin) isAdminTarget = true; // "admin" or "superadmin"
    }
    ...
    if (isAdminTarget) {
      await react("❌");
      return reply(
        `❌ @${targetNum} is a group admin and cannot be kicked.\nDemote them first with *.demote* if you still want to remove them.`,
        {
          mentions: [targetJid],
          contextInfo: { mentionedJid: [targetJid] },
        },
